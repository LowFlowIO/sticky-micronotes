...
void EInkDisplay::begin() {
  ...
  SPI.begin(_sclk, SPI_MISO, _mosi, _cs);
#ifdef EPD_SPI_HZ
  spiSettings = SPISettings(EPD_SPI_HZ, MSBFIRST, SPI_MODE0);
#else
  spiSettings = SPISettings(40000000, MSBFIRST, SPI_MODE0);
#endif
  if (Serial) Serial.printf("[%lu]   SPI initialized at %u Hz, Mode 0\n", millis(),
#ifdef EPD_SPI_HZ
                            (unsigned)EPD_SPI_HZ
#else
                            40000000u
...
void EInkDisplay::refreshDisplay(const RefreshMode mode, const bool turnOffScreen) {
  ...
  if (mode == FULL_REFRESH) {
    sendCommand(CMD_BORDER_WAVEFORM);
    sendData(0x01);
    displayMode = 0xF7;  // Seeed vendor FULL sequence (self-powers)
    ...
    sendCommand(CMD_BORDER_WAVEFORM);
    sendData(0x80);  // vendor PARTIAL/DU border — avoids a dark ring
    displayMode = customLutActive ? 0x0C : 0xFF;
  }
